...
class PDF extends FPDF
{
        function Header()
        {
            ...
            $this->MultiCell(220, 20, 'REPORTE MANTENIMIENTOS POR CLIENTE ', 0, 'C', 0,);
        ...
        function Footer()
        {
            ...
            $select = $pdo->prepare("SELECT * FROM mantenimiento WHERE doc_cli='1094929643'");
            $select->execute();
            while($row=$select->fetch(PDO::FETCH_OBJ))
            {
                $this->Cell(38,7,$row->id_man,1);
                $this->Cell(38,7,$row->fec_man,1);
                $this->Cell(38,7,$row->equ_man,1);
                $this->Cell(38,7,$row->ser_equ,1);
                $this->Cell(38,7,$row->fal_equ,1);
                $this->Ln();
            }
            ...
            function Tabla()
            {
                ...
                $this->MultiCell(200,3,  'Datos basicos del cliente:', 0, 'L', 0);
            } 
            function mensaje1()
            {
                $this->SetFont('Arial', '', 12);
            ...
            function datos(){
                ...
                $select = $pdo->prepare("SELECT * FROM personas WHERE doc_per='1094929643'");
                $select->execute();
                ...
                $this->SetFont('Arial', '', 7);
                $this->SetX(38.5);
                $this->MultiCell(53, 4, $row->doc_per, 0, 'L', 0);
                ...
                $this->SetFont('Arial', '', 7);
                $this->SetX(38.5);
                $this->MultiCell(53, 4, $row->nom_per, 0, 'L', 0);
                ...
                $this->SetFont('Arial', '', 7);
                $this->SetX(38.5);
                $this->MultiCell(53, 4, $row->ape_per, 0, 'L', 0);
                ...
                $this->Multicell(78, 4, $row->tel_per, 0, 'L', 0);
    
                $this->SetY(51.8);
                $this->SetFont('Arial', 'B', 7);
                $this->setFillColor(230, 230, 230);
                $this->SetX(101);
                $this->Cell(20, 6, 'DIRECCION: ', 8, 8, 'L', 'TRUE');
                $this->SetY(52.5);
                $this->SetFont('Arial', '', 7);
                //$pdf->Cell(218, 4, $row->mar_equ, 8, 8, 'C');
                $this->SetX(121);
                $this->MultiCell(79, 4, $row->dir_per, 0, 'L', 0);
    
                $this->SetY(57.5);
                $this->SetFont('Arial', 'B', 7);
                $this->setFillColor(230, 230, 230);
                $this->SetX(101);
                $this->Cell(20, 6, 'CORREO: ', 8, 8, 'L', 'TRUE');
                $this->SetY(58.5);
                $this->SetFont('Arial', '', 7);
                //$pdf->Cell(240, 4, $row->ref_equ, 8, 8, 'C');
                $this->SetX(121);
                $this->MultiCell(72, 4, $row->cor_per, 0, 'L', 0);
            }
}
    $pdf=new PDF();
