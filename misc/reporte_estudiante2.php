...
class PDF extends FPDF
{
        function Header()
        {
            ...
            $this->MultiCell(220, 20, 'REPORTE ALUMNOS INACTIVOS ', 0, 'C', 0,);
            ...
            $this->Cell(32,5,$header[$i],1,0,'C',1);
            $this->Ln();
            ...
            $select = $pdo->prepare("SELECT * FROM alumno WHERE estado = 0");
            $select->execute();
            while($row=$select->fetch(PDO::FETCH_OBJ))
            {
                $this->Cell(32,7,$row->id_Alumno,1);
                $this->Cell(32,7,$row->Nombre,1);
                $this->Cell(32,7,$row->Apellido,1);
                $this->Cell(32,7,$row->Telefono,1);
                $this->Cell(32,7,$row->estado,1);
                $this->Cell(32,7,$row->Grado,1);
                $this->Ln();
            }
            ...
            function datos(){
                ...
                $this->SetX(38.5);
                $this->MultiCell(53, 4, $row->id_Alumno, 0, 'L', 0);
                ...
                $this->SetX(121);
                $this->MultiCell(72, 4, $row->Grado, 0, 'L', 0);
            }
}
    $pdf=new PDF();
    $header=array('ID','NOMBRE','APELLIDO','TELEFONO','ESTADO','GRADO');
    $pdf->AliasNbPages();
    $pdf->AddPage();
